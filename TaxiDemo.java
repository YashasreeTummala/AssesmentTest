...
{	public void select() throws Exception
	...
	public void calculate(int choice, String type, int distance) throws Exception, IOException
	{
		...
		if(choice == 1 && distance <75)
		{
			if(distance > 3)
			{
				int tempDistance = distance -3;	 //d = 20 t = 17
				finalCost = 50;				
				finalCost += tempDistance *10;   
				System.out.println("Final Price to be paid is: " + finalCost + "\n");
				select();
			}
		}
		...
		{
			if(distance < 5) 
				System.out.println("Final Price to be paid is: " + finalCost*distance + "\n");
			else if(distance > 5)
			{
				int tempDistance = distance -5;
				finalCost = 100;
